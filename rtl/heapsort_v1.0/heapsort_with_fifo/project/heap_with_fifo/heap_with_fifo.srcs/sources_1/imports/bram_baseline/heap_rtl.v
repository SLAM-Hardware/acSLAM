...
//解析fifo读信号，外部输入时，不断比较读出数据和堆顶，若丢弃就继续读；fifo输入时，每两个时钟周期读一次
    always @(posedge clk or negedge rstn) begin
        if(~rstn)
            dout_buf1 <= 0;
        else
...
//描述数据输入，暂时不考虑外部输入时需要切到fifo输入的情况
//ext = 1,则din1din2从din切换输入；fifo = 1，则当fifo_en1时din_1 = dout_buf1。
    always @(posedge clk or negedge rstn) begin
        if(~rstn) begin
            din_1 <= 0;
            din_2 <= 0;
